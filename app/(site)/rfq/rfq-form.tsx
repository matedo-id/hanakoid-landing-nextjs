...
export function RfqForm({
  ...
  const [items, setItems] = useState<Item[]>([
    newItem(
      initialProduct && products.some((p) => p.id === initialProduct)
        ? initialProduct
        : ""
    ),
  ]);
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const leadItems = filledItems.map((i) => ({
      product:
    ...
    const payload: LeadPayload = {
      ...
      instansiType: data.get("type")?.toString() || undefined,
      timeline: data.get("timeline")?.toString() || undefined,
      budget: data.get("budget")?.toString().trim() || undefined,
      location: data.get("location")?.toString().trim() || undefined,
      message: notes || undefined,
      items: leadItems.length > 0 ? leadItems : undefined,
      source: solution ? `rfq-solusi:${solution.key}` : "rfq-page",
  ...
  if (submitted) {
    return (
      <FormSuccess
        ...
        message="Terima kasih. Tim sales kami akan menindaklanjuti penawaran Anda. Untuk respons lebih cepat, lanjutkan ke WhatsApp."
        waUrl={waUrl}
        ...
        {solution && (
          <div className="card flex items-center gap-3 bg-primary-050 p-4">
            ...
            <p className="text-sm text-primary-dark">
              Penawaran untuk paket solusi: <strong>{solution.title}</strong>
            </p>
          </div>
        )}
